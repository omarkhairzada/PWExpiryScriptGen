...
.TEMPLATE INFORMATION 
	Change the desired domain and OU starting in line 141, utilizing the -searchbase parameter.
	OU can be entered in place of OU=NYLAG - USers, in format: OU=___
	For example, inputting OU "NYLAG - Users" would require format: OU=NYLAG - Users
	Domain is broken up by the delimiter "." and replaces DC=tech,DC=nylag,DC=org in format: DC=___,DC=___
	For example, inputting domain tech.nylag.com would require format: DC=tech,DC=nylag,DC=org
	You may also choose to change the email message in HTML format, starting line 218
.Synopsis
...
$smtpServerLabel = "nylag-org.mail.protection.outlook.com".PadRight($padVal," ")
$expireInDaysLabel = "Expire in Days".PadRight($padVal," ")
$fromLabel = "itsupport@nylag.org".PadRight($padVal," ")
...
#OU=NYLAG - USers and DC=tech,DC=nylag,DC=org are PLACEHOLDER Names for what will be replaced. Replaces OU and Domain
$users = get-aduser -filter {(Enabled -eq $true) -and (PasswordNeverExpires -eq $false)} -searchbase "OU=NYLAG - USers,DC=tech,DC=nylag,DC=org" -properties Name, PasswordNeverExpires, PasswordExpired, PasswordLastSet, EmailAddress | where { $_.passwordexpired -eq $false }
# Count Users
...
foreach ($user in $notifyUsers)
{
    ...
    <p> To avoid getting locked out of the system, please change your password as soon as possible. If your password expires before you change your password, the system will not allow you to log in. You will need to request assistance from IT in order to log back in. <br>
    <p> If you are logging into a computer in the office, after logging in you will see a prompt to change your password. <br>
    <p> If you are logging into the remote server, below is how you change your password.
		<p> <strong>Using a Windows Computer</strong> - Once signed into the remote server, hit <strong>ctrl-alt-end</strong> and then select change a password. <br>
		<p> <strong>Using a MAC</strong> - Once signed into the remote server, hit <strong>fn-ctrl-option-delete</strong>, and then select change a password. <br>
	<p> This email purposely does not have any links to avoid any confusion with Phishing emails. If you need any assistance with changing your password, please reply to this email or send a new email to ITSupport@nylag.org directly. <br>
    <p> Don't forget to Update the password on your Mobile Devices as well!
    <p>Thanks, <br> 
    </P>
    IT Support
    </font>"
    # If Testing Is Enabled - Email Administrator   - <a href=""mailto:support@domain.com""?Subject=Password Expiry Assistance"">support@domain.com</a> | 0123 456 78910
    if($testing)
    {
...
if($logging)
{
    ...
    $year = $today.Year
    $date = "$day-$month-$year"
    $logFileName = "$date-PasswordLog-Users.csv"
    if(($logPath.EndsWith("\")))
    {
