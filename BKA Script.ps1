...
.TEMPLATE INFORMATION 
	...
	OU can be entered in place of OU=BKA - Users, in format: OU=___
	For example, inputting OU "NYLAG - Users" would require format: OU=NYLAG - Users
	Domain is broken up by the delimiter "." and replaces DC=test,DC=bka,DC=org in format: DC=___,DC=___
...
#OU=BKA - Users and DC=test,DC=bka,DC=org are PLACEHOLDER Names for what will be replaced. Replaces OU and Domain
$users = get-aduser -filter {(Enabled -eq $true) -and (PasswordNeverExpires -eq $false)} -searchbase "OU=BKA - Users,DC=test,DC=bka,DC=org" -properties Name, PasswordNeverExpires, PasswordExpired, PasswordLastSet, EmailAddress | where { $_.passwordexpired -eq $false }
# Count Users
