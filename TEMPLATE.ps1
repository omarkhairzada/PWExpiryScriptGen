...
.TEMPLATE INFORMATION 
	...
	OU can be entered in place of OUCHANGE, in format: OU=___
	For example, inputting OU "NYLAG - Users" would require format: OU=NYLAG - Users
	Domain is broken up by the delimiter "." and replaces DCCHANGE in format: DC=___,DC=___
...
#OUCHANGE and DCCHANGE are PLACEHOLDER Names for what will be replaced. Replaces OU and Domain
$users = get-aduser -filter {(Enabled -eq $true) -and (PasswordNeverExpires -eq $false)} -searchbase "OUCHANGE,DCCHANGE" -properties Name, PasswordNeverExpires, PasswordExpired, PasswordLastSet, EmailAddress | where { $_.passwordexpired -eq $false }
# Count Users
